...
import { exhanoi_2 } from './agent';

export default function Exhanoi2() {
  ...
  useEffect(() => {
    ...
    if (diskNum > 0) {
      const movesCp = [];
      exhanoi_2('A', 'B', 'C', diskNum, movesCp);
      setMoves(movesCp);
      setMovesB([]);
    }
  }, [diskNum, reset]);
  ...
  const initializeDisks = (num) => {
    ...
    setDisks({ A: initialDisksA, B: initialDisksB, C: [] });
  };
