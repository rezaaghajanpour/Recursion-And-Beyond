...
import { exhanoi_4 } from './agent';

export default function Exhanoi3() {
  ...
  useEffect(() => {
    ...
    if (diskNum > 0) {
      const movesCp = [];
      exhanoi_4('A', 'B', 'C', diskNum, movesCp);
      setMoves(movesCp);      
      console.log(movesCp,length);
  ...
  const initializeDisks = (num) => {
    ...
    for (let i = 0; i < 6*num; i++) {
        const size = num > 10 ? num * 14 - (i + 1) * 10 : 300 - (i + 1) * 10;
        if(i % 6 == 0 || i % 6 == 1 || i % 6 == 2)
          initialDisksA.push({ size, num: i + 1 });
        else if(i % 6 == 3 || i % 6 == 4)
          initialDisksB.push({ size, num: i + 1 });
        else
          initialDisksC.push({ size, num: i + 1 });
    }
    setDisks({ A: initialDisksA, B: initialDisksB, C: initialDisksC });
  };
